                ...
                @elseif (count($customFilters))
                    @foreach ($customFilters as $key => $filter)
                        <div wire:key="filter-{{ $key }}" class="p-2">
                            ...
                            @if ($filter->isSelect())
                                @include('livewire-tables-v1::bootstrap-5.includes.filter-type-select')
                            @elseif($filter->isMultiSelect())
                                @include('livewire-tables-v1::bootstrap-5.includes.filter-type-multiselect')
                            @elseif($filter->isDate())
                                @include('livewire-tables-v1::bootstrap-5.includes.filter-type-date')
                            @elseif($filter->isDatetime())
                                @include('livewire-tables-v1::bootstrap-5.includes.filter-type-datetime')
                            @endif
                        </div>
                    @endforeach
                @endif
